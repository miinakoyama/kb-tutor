...
const STIMULUS_DESCRIPTIONS: Record<StimulusType, string> = {
  table: DIAGRAM_DESCRIPTIONS.table,
  line_graph: `A line graph showing a trend or relationship. Generate data as:
{
  "type": "chart",
  "data": {
    "chartType": "line",
    ...
    "xAxisLabel": "X Axis Label",
    "yAxisLabel": "Y Axis Label",
    "series": [{ "key": "seriesA", "label": "Series A" }],
    "data": [
      { "x": "Condition 1", "seriesValues": { "seriesA": 12 } },
      { "x": "Condition 2", "seriesValues": { "seriesA": 18 } }
    ]
  }
}
Use a line graph only. Do not output a bar chart.`,
  bar_chart: `A bar chart showing category comparisons. Generate data as:
{
  "type": "chart",
  "data": {
    "chartType": "bar",
...
Use a bar chart only. Do not output a line graph.`,
  diagram: DIAGRAM_DESCRIPTIONS.diagram,
  scenario:
    'A text-only scenario, experiment, observation, claim, or model description embedded in the question stem. The question must have "diagram": null.',
  illustration:
    'An illustration-style biology visual generated by the image model after this text call. Do not create SVG. Set "diagram": null and "imageUrl": null. Include an "illustrationPrompt" field with a concise black-and-white worksheet illustration prompt for the image model.',
};
...
Use this exact stimulus type for the question: ${settings.stimulusType}

${STIMULUS_DESCRIPTIONS[settings.stimulusType]}

For visual stimulus types, the question text should reference the stimulus. For scenario, write the scenario in the stem and set "diagram": null.
`;
  } else if (settings.includeDiagrams) {
    const diagramTypes: { type: DiagramType; count: number }[] = [];
    const diagramConfig = settings.diagramConfig ?? {
      chart: 0,
      table: 0,
      flowchart: 0,
      diagram: 0,
    };
    if (diagramConfig.chart > 0) {
      diagramTypes.push({ type: "chart", count: diagramConfig.chart });
    }
    if (diagramConfig.table > 0) {
      diagramTypes.push({ type: "table", count: diagramConfig.table });
    }
    if (diagramConfig.flowchart > 0) {
      diagramTypes.push({
        type: "flowchart",
        count: diagramConfig.flowchart,
      });
    }
    if (diagramConfig.diagram > 0) {
      diagramTypes.push({
        type: "diagram",
        count: diagramConfig.diagram,
      });
    }
...

{
  ...
  "kcCode": "Selected KC code",
  "kcStatement": "Selected KC statement",
  "commonMisconception": "A common mistake students make related to this topic",
  "illustrationPrompt": "Only when stimulus type is illustration: prompt for a black-and-white worksheet image; otherwise omit",
  "rationaleQuestion": {
