...
function AssignmentCard({
  ...
  return (
    <article className="rounded-2xl border border-primary/25 bg-surface p-5 sm:p-6 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-col gap-2 w-full sm:w-[11.5rem] flex-shrink-0">
          {isRestartBlocked ? (
            ...
            <Link
              href={href}
              className="inline-flex items-center justify-center gap-2 w-full px-4 py-2.5 min-h-[44px] rounded-lg bg-primary text-white text-sm font-medium hover:bg-primary-hover transition-colors"
            >
...
function StatusBadge({ status }: { status: StudentAssignmentStatus }) {
  if (status === "completed") {
    return (
      <span className="inline-flex items-center gap-1 text-xs font-semibold text-heading bg-primary/15 px-2 py-1 rounded-full">
