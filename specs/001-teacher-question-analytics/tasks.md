...
- [ ] T001 Verify project skeleton is healthy: run `npm install`, `npm run lint`, and `npm test` from repo root. Fix any pre-existing failures or document them in the PR description as not blocking.
- [ ] T002 Create empty directory scaffolds (no-op `.gitkeep` files where needed) so subsequent tasks have stable paths: `src/app/teacher-dashboard/standards/`, `src/app/teacher-dashboard/students/`, `src/app/api/teacher-dashboard/standards/`, `src/app/api/teacher-dashboard/students/`, `src/app/api/teacher-dashboard/questions/`, `src/components/teacher/`.
